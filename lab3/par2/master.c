...
void do_master_stuff(int argc, char ** argv, struct mw_api_spec *f)
{
  ...
  while(num_results_received < num_work_units)
  {
    ...
    //remind supervisor i am still working
    MPI_Send(0, 0, MPI_INT, 1, M_PING_TAG, MPI_COMM_WORLD);
  ...
  //send message to supervisor that im about to compute results
  MPI_Send(1, 1, MPI_INT, 1, M_PING_TAG, MPI_COMM_WORLD);
