...
void do_supervisor_stuff(int argc, char ** argv, struct mw_api_spec *f)
{
  ...
  MPI_Irecv(assignment_time1, number_of_slaves, MPI_DOUBLE, master_id, SUPERVISOR_TAG, MPI_COMM_WORLD, &master_request);
  ...
  if(!master_started_flag)
  {
    ...
    MPI_Send(&master_id, 1, MPI_INT, master_id, KILL_TAG, MPI_COMM_WORLD);
    master_id = 2;
    MPI_Send(&master_id, 1, MPI_INT, master_id, START_TAG, MPI_COMM_WORLD);
    MPI_Recv(assignment_time1, number_of_slaves, MPI_DOUBLE, master_id, MPI_ANY_TAG, MPI_COMM_WORLD, &master_status);
  }
  ...
  //if the master hasnt failed, we will be checking for it
  int master_ping=0;
  MPI_Irecv(&master_ping, 1, MPI_INT, master_id, M_PING_TAG, MPI_COMM_WORLD, &master_request);

  //set up receivers for kill and master updates
  MPI_Irecv(&kill_msg, 1, MPI_INT, master_id, KILL_TAG, MPI_COMM_WORLD, &request1);
  MPI_Irecv(assignment_time2, number_of_slaves, MPI_DOUBLE, master_id, SUPERVISOR_TAG, MPI_COMM_WORLD, &request2);
  ...
  while(1)
  {
    ...
    for(i=0; i<number_of_slaves; i++)
    {
      ...
      else if(threshold>0 && assignment_time1[i]==assignment_time2[i] && MPI_Wtime() - mytime_off_by - assignment_time1[i] > threshold)
      {
        ...
        MPI_Send(&i, 1, MPI_INT, master_id, FAIL_TAG, MPI_COMM_WORLD);
        failed_worker[i] = 1;
      }
    }
    if(received_update)
